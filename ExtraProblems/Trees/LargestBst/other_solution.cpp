...
bool isBST(TreeNode *root, int mn, int mx, int &nodeCount){
    if(root == NULL){
        return true;
    }
    ...
    nodeCount++;
    return (isBST(root->left_ptr, mn, root->val, nodeCount) && isBST(root->right_ptr, root->val, mx, nodeCount));
}
...
int findLargestBST(TreeNode *root) {
    ...
    int nodeCount = 0;
    if(isBST(root, -1000000009, 1000000009, nodeCount)){
        return nodeCount;
    }
    return max(findLargestBST(root->left_ptr), findLargestBST(root->right_ptr));
}
